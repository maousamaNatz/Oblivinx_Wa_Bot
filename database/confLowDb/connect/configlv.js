...
async function initializeDatabase() {
  try {
    ...
    console.error("Error initializing level config database:", error);
  }
}
...
async function readDatabase() {
  try {
    ...
    console.error("Error reading level config database:", error);
    return null;
  }
}
...
async function writeDatabase(data) {
  try {
    ...
    console.error("Error writing to level config database:", error);
    return false;
  }
}
...
async function getLevelConfig(level) {
  ...
  const data = await readDatabase();
  return data.level_config.find((config) => config.level === level);
}
...
async function addLevelConfig(levelData) {
  try {
    if (!validateLevelConfig(levelData)) {
      throw new Error(
        "Invalid level config data: " +
          ajv.errorsText(validateLevelConfig.errors)
      );
    }
    ...
    if (exists) return { success: false, message: "Level already exists" };
    ...
    console.error("Error adding level config:", error);
...
async function updateLevelConfig(level, levelData) {
  try {
    ...
    if (!config) return { success: false, message: "Level not found" };
...
async function deleteLevelConfig(level) {
  try {
    ...
    console.error("Error deleting level config:", error);
    return { success: false, message: error.message };
  }
}
