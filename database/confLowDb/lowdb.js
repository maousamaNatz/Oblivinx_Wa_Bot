...
async function addGroup(groupData) {
  try {
    ...
    const ownerExists = await checkForeignKey("users", "user_id", groupData.owner_id);
    if (!ownerExists) {
      throw new Error(`Owner_id ${groupData.owner_id} does not exist in users collection`);
    }
